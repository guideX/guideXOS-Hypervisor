...
|                         VMManager                          |
|  +----------------+  +----------------+  +----------------+ |
|  | VMInstance #1  |  | VMInstance #2  |  | VMInstance #n  | |
|  | +------------+ |  | +------------+ |  | +------------+ | |
...
This diagram shows that VMManager hosts multiple VM instances. Each VM instance contains a VirtualMachine which includes:

- CPUs (one or more virtual CPU contexts)
- Memory + MMU (private 64-bit address space)
- Virtual devices (console, timer, disk, etc.)

The text above and the rest of this README describe how those components interact.
...
- `test_fuzzer.cpp` - Fuzzing framework tests
