...
bool MTypeDecoder::decode(uint64_t raw_instruction, formats::MFormat& result) {
        ...
        if (major == 0x0 && x3 == 0x0 && (x4 == 0x6 || x4 == 0x7)) {
            ...
            result.operation = x4 == 0x6
                ? formats::MFormat::MemOp::SSM
                : formats::MFormat::MemOp::RSM;
            result.has_imm = true;
...
bool MTypeDecoder::toInstruction(const formats::MFormat& fmt, InstructionEx& instr) {
        ...
        if (fmt.operation == formats::MFormat::MemOp::LOAD) {
            ...
            instr = InstructionEx(fmt.release ? InstructionType::FETCHADD4_REL
                                              : InstructionType::FETCHADD4_ACQ,
                                  UnitType::M_UNIT);
            instr.SetPredicate(fmt.qp);
